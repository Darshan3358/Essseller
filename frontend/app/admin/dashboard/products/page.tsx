...
function AdminProductsPage() {
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Immediately show a quick preview while we compress
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const data = res.headers.get('content-type')?.includes('application/json') 
                ? await res.json() 
                : { message: `Server error: ${res.status} ${res.statusText}. Please check the file size.` };

            if (!res.ok) {
                if (res.status === 413) {
                    throw new Error('Image file is too large for the server. Vercel limit is 4.5MB. Please resize your image.');
                }
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
                ...
                >
                    ...
                    {categories.map((c, idx) => (
                        <option key={idx} value={typeof c === 'object' ? c.name : c} style={{ background: '#1e293b' }}>
                    ...
                    {products.map(p => (
                        ...
                        >
                            ...
                            <div style={{ height: '150px', overflow: 'hidden', background: 'rgba(255,255,255,0.03)', position: 'relative' }}>
                                {p.image ? (
                                    <img
                                        src={p.image.startsWith('http') ? p.image : (p.image.startsWith('/') ? p.image : `/uploads/${p.image}`)}
                                        alt={p.name}
                                            ...
                                            <div style={{ position: 'relative', flex: 1 }}>
                                                <input
                                                    ...
                                                    style={{
                                                        ...
                                                        background: 'rgba(255,255,255,0.07)', border: '1px solid #3b82f6',
                                            ...
                                            <>
                                                <select
                                                    ...
                                                    style={{
                                                        ...
                                                        background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)',
                                                        borderRadius: '10px', color: form.category ? 'white' : 'rgba(255,255,255,0.3)', 
                                                ...
                                                >
                                                    ...
                                                    {categories.map((c, idx) => {
                                                        ...
                                                        return <option key={idx} value={name} style={{ background: '#0f172a' }}>{name}</option>;
                                                    })}
                                                </select>
                                                <button
                                                    type="button"
