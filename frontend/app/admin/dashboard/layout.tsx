...
import {
    ...
    ShoppingCart, Box, LogOut, Shield, Menu, X, ChevronRight, Settings, Sparkles, LifeBuoy, History
} from 'lucide-react';
...
const navItems = [
    ...
    { label: 'Spread Purchases', icon: History, href: '/admin/dashboard/spread-packages/purchases' },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (!host.includes('nxgridpxcnode91') && !host.includes('localhost') && !host.includes('127.0.0.1')) {
                router.replace('/');
                return;
            }
        }
