...
export default function AdminOrdersPage() {
    ...
    const [orderForm, setOrderForm] = useState({
        ...
        customer_address: '', payment_method: 'COD', supplier_name: ''
    });
    ...
    const openAddModal = async () => {
        ...
        setOrderForm({ customer_name: '', customer_email: '', customer_phone: '', customer_address: '', payment_method: 'UPI', supplier_name: '' });
        setFormError('');
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            const res = await fetch(`${API}/admin/orders/${editModal._id}`, {
                ...
                body: JSON.stringify({ 
                    status: editStatus, 
                    pick_up_status: editPickUpStatus,
                                                ...
                                                <div style={{ display: 'flex', gap: '6px' }}>
                                                    ...
                                                    <button 
                                                        onClick={() => { 
                                                            ...
                                                            setEditPickUpStatus(o.pick_up_status || 'Unpicked-Up'); 
                                                            setEditSupplierName(o.supplier_name || '');
                                                        }} 
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ padding: '28px' }}>
                            ...
                            {/* Section: Seller & Supplier */}
                            <div style={{ marginBottom: '32px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                                    <div style={{ width: '3px', height: '18px', background: 'linear-gradient(135deg, #3b82f6, #60a5fa)', borderRadius: '2px' }} />
                                    <span style={{ fontSize: '13px', fontWeight: '800', color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Seller & Supplier</span>
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ position: 'relative' }}>
                                            <select
                                                ...
                                                onChange={e => setOrderForm(f => ({ ...f, supplier_name: e.target.value }))}
                                                style={{ ...inputStyle, paddingRight: '36px', appearance: 'none', cursor: 'pointer' }}
                                            >
                                                <option value="">— Select Supplier (Auto/Default) —</option>
                                                {suppliers.map(s => (
                                                    <option key={s._id} value={s.name} style={{ background: '#1e3a8a' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            {(() => {
                                ...
                                if (viewModal.order.status === 'delivered' && viewModal.order.deliveredAt) {
                                    meta.push({ 
                                        label: 'Delivered', 
                                        value: new Date(viewModal.order.deliveredAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) + ' ' + 
                                               new Date(viewModal.order.deliveredAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true })
                                    });
                                }
                                return meta.map((m, idx) => (
                                    <div key={idx} style={{
                                        flex: 1, padding: '12px 16px',
                                        borderRight: idx < meta.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none',
                                        textAlign: 'center'
                                    }}>
                                        ...
                                        <div style={{ fontSize: '13px', fontWeight: '700', color: m.label === 'Delivered' ? '#10b981' : 'white', textTransform: 'capitalize' }}>{m.value}</div>
                                    </div>
                                ));
                            })()}
                        </div>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            <label style={labelStyle}>Assign Supplier</label>
                            <div style={{ position: 'relative' }}>
                                <select
                                ...
                                >
                                    ...
                                    {suppliers.map(s => (
                                        <option key={s._id} value={s.name}>{s.name} ({s.location})</option>
                                    ))}
                                </select>
                                <ChevronDown size={14} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.4)', pointerEvents: 'none' }} />
                            </div>
