...
import { Box, Sparkles, Zap, ArrowUpRight, ShieldCheck, Star, Loader2, X, Lock, ArrowRight, CheckCircle2 } from 'lucide-react';
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                if (res.spread_package) {
                    updateUser({ spread_package: res.spread_package });
                }
            } else {
                        ...
                        {packages.map((pkg) => (
                            <div key={pkg._id} className={`relative group flex flex-col ${pkg.popular ? 'scale-105 z-10' : ''}`}>
                                ...
                                <div className={`glass-card p-10 h-full flex flex-col border-2 transition-all duration-500 backdrop-blur-3xl overflow-hidden ${pkg.popular
                                    ...
                                    <button
                                        onClick={() => !((user?.spread_package === pkg._id) || (user?.spread_package === pkg.id)) && openPurchaseModal(pkg)}
                                        disabled={(user?.spread_package === pkg._id) || (user?.spread_package === pkg.id)}
                                        className={`w-full py-5 rounded-[1.5rem] font-black transition-all flex items-center justify-center gap-3 relative overflow-hidden group/btn ${((user?.spread_package === pkg._id) || (user?.spread_package === pkg.id))
                                            ? 'bg-emerald-500 text-white shadow-xl shadow-emerald-200 dark:shadow-emerald-900/20 cursor-default'
                                            : pkg.popular
                                        ...
                                        <span className="relative z-10">
                                            {((user?.spread_package === pkg._id) || (user?.spread_package === pkg.id)) ? t('Activated') : t('Select Package')}
                                        </span>
                                        {((user?.spread_package === pkg._id) || (user?.spread_package === pkg.id)) ? (
