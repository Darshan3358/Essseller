...
export default function OrderDetailsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (orderRes) {
                    setOrder(orderRes);
                    setSelectedSupplier(orderRes.supplier_name || '');
                }
    ...
    const handlePayStorehouse = async () => {
        ...
        try {
            await api.put(`/orders/${id}/pay-storehouse`, { 
                trans_password: transPassword,
                supplier_name: selectedSupplier || order.supplier_name
            });
            setPaySuccess(true);
            setPayStep('success');
                        ...
                        <div className="glass-card p-8 !bg-white/60 dark:!bg-slate-900/60">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className={`text-xs font-medium uppercase tracking-widest ${(order.pick_up_status?.toLowerCase().includes('picked') && !order.pick_up_status?.toLowerCase().includes('unpicked'))
                                        ? 'text-black dark:text-slate-100'
                                        : 'text-red-600 dark:text-red-400'
                                        }`}>
                                        {(order.pick_up_status?.toLowerCase().includes('picked') && !order.pick_up_status?.toLowerCase().includes('unpicked')) ? t('Picked') : t('Unpicked')}
                                    </span>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className={`text-sm font-black ${order.status === 'delivered' ? 'text-success-600 dark:text-success-400' : 'text-gray-400'}`}>
                                            {order.deliveredAt ? new Date(order.deliveredAt).toLocaleString() : t('Pending')}
                                        </span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <>
                                        ...
                                        <div className="space-y-6 pt-2 pb-2">
                                            ...
                                            <div className="space-y-2">
                                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.15em] px-1">{t('Transaction Password')}</label>
                                                <div className="relative group">
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder="••••••••"
                                                        className="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 font-bold text-gray-900 transition-all placeholder:text-gray-300"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            {/* Supplier Selection */}
                                            <div className="space-y-2">
                                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] px-1">{t('Assign Supplier')}</label>
                                                <div className="relative group/select">
                                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                                        <Truck size={16} className="text-gray-400 group-focus-within/select:text-primary-500 transition-colors" />
                                                    </div>
                                                    <select
                                                        value={selectedSupplier}
                                                        onChange={(e) => setSelectedSupplier(e.target.value)}
                                                        className="w-full pl-11 pr-10 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 font-bold text-gray-900 appearance-none transition-all cursor-pointer"
                                                    >
                                                        <option value="">{t('Select Supplier...')}</option>
                                                        {suppliers.map(s => (
                                                            <option key={s._id} value={s.name}>{s.name} ({s.deliveryTimeEstimate})</option>
                                                        ))}
                                                    </select>
                                                ...
                                                <p className="text-[10px] text-gray-400 px-1 italic">{t('Choose a fulfillment partner for this order.')}</p>
                                            </div>
