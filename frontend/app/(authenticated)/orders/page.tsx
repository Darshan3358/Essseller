                                                    ...
                                                    <tr key={`${order._id}-expanded`} className="bg-primary-50/40 dark:bg-primary-900/10">
                                                        <td colSpan={4} className="px-4 pb-3 pt-1">
                                                            <div className="flex flex-wrap gap-x-6 gap-y-1.5 text-xs">
                                                                <div className="text-left">
                                                                    <span className="text-gray-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{order.status === 'delivered' ? t('Delivery Date') : t('Order Date')}</span>
                                    ...
                                    (orders || []).map((order) => (
                                        <tr key={order._id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                {order.status === 'delivered' && order.deliveredAt ? (
                                                    <>
                                                        <p className="text-sm font-bold text-success-600 dark:text-success-400">{new Date(order.deliveredAt).toLocaleDateString()}</p>
                                                        <p className="text-[11px] text-success-500/70 dark:text-success-500/50 font-bold mt-0.5">{new Date(order.deliveredAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                                        <p className="text-[9px] text-gray-400 uppercase tracking-tighter mt-1">{t('Delivered')}</p>
                                                    </>
                                                ) : (
                                                    <>
