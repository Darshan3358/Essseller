...
const updateCryptoSettings = asyncHandler(async (req, res) => {
    const { usdt_trc20, usdt_erc20, btc, bnb, qr_code_url, network_note, min_deposit } = req.body;

    const value = { usdt_trc20, usdt_erc20, btc, bnb, qr_code_url: qr_code_url || '', network_note, min_deposit };
...
const updateBankSettings = asyncHandler(async (req, res) => {
    const { bank_name, account_name, account_number, ifsc_code, branch, swift_code, qr_code_url, note } = req.body;
    const value = { bank_name, account_name, account_number, ifsc_code, branch, swift_code, qr_code_url: qr_code_url || '', note };
    await SiteSetting.findOneAndUpdate(
        { key: 'bank_payment' },
