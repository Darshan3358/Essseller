...
const purchaseSpreadPackage = asyncHandler(async (req, res) => {
    ...
    // 3. Logic to "apply" the package
    seller.spread_package = pkg._id;
    
    // Record as a Package expense so getAvailableBalance handles it
    const Package = require('../models/Package');
    await Package.create({
        id: Math.floor(100000 + Math.random() * 900000),
        seller_id: seller._id,
        type: `Spread: ${pkg.name}`,
        amount: pkg.price,
        profit: '0',
        product_limit: 0,
    ...
    // Also deduct from cached wallet_balance if it exists
    if (typeof seller.wallet_balance === 'number') {
        seller.wallet_balance -= pkg.price;
    }
